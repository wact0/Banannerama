...
class Outline {
	...
	public:
		...
		bool intersects(Outline a) {
			...
			// if(((a.x1() > x1() && a.x1() < x2()) && a.x2() > x2()) || ((a.x2() < x2() && a.x2() > x1()) && a.x1() < x1())) {
			// 	response = true;
			// }
			return response;
		}
...
vector<string> comparediff(vector<Outline> voutline) {
	...
	for(int i=1;i<voutline.size()-1;i++) {
		...
		// }
		//-----------//
		// if(voutline[i].x2() > voutline[lindex].x2()) {
		// 	if(voutline[i].x1() > voutline[lindex].x2()) {
		// 		res.push_back("("+to_string(voutline[lindex].x2())+",0)");
		// 		res.push_back("("+to_string(voutline[i].x1()) + "," + to_string(voutline[i].height())+")");
		// 	} else {
		// 		res.push_back("("+to_string(voutline[lindex].x2())+","+to_string(voutline[i].height())+")");
		// 	}
		// 	lindex = i+1;
		// } else {
		// 	//check if the second one's height is greater than first's, add the second one.
		// 	//else because
		// }
		//-----------//
		if(voutline[i].x2() > voutline[lindex].x2()) {
			if(voutline[i].x1() > voutline[lindex].x2()) {
				...
				if(voutline[i].height() > voutline[lindex].height()) {
					res.push_back("("+to_string(voutline[lindex].x2())+","+to_string(voutline[i].height())+")");
				}
			}
